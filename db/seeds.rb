...
tenant_oids = Tenant.only(:_id).map(&:_id)
...
User.create! name: "sa",
  ...
  tenant_oid: Tenant.first._id, # this is not used for site admins
  role_id: ROLE_ADMIN

User.create! name: "ta",
  password: "f",
  password_confirmation: "f",
  tenant_oid: Tenant.first._id,
...
# that are from their own tenancy), and regular users (who have a tenant_oid,
# and whose content are always associated with this tenant). The tenant_oid only
# matters for tenant admins and regular users (site admins overrule all tenants,
# so it does not make sense for them to have a tenant id).
FAKE_USERS_CNT.times do
  ...
  User.create!\
    ...
    tenant_oid: tenant_oids[rand FAKE_TENANTS_CNT],
